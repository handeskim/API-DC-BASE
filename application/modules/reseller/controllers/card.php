...
class Card extends MY_Controller{
	...
	public function info($id){
		if($this->_role == 1 || $this->_role == 2 || $this->_role ==3){
			if(isset($id)){
				...
				$this->obj['keys'] = $id;
				$this->result = $this->GlobalMD->query_result('api/card_cms/info',$this->obj);
				if(isset($this->result->result)){
					if(!empty($this->result->result)){ $this->data['card'] = convert_object($this->result->result);}
				}
				$this->data['root_id'] = $id;
			}
			$this->data['title'] = lang('card');
			$this->data['title_main'] = 'Detail Transaction Card ID#'.$id;
			$this->data['side_bar'] = 4;
			$this->parser->parse('reseller/layout/apps/card/card_info',$this->data);
		}else{
			$this->is_profile();
		}
	}
	public function index(){
		if($this->_role == 1 || $this->_role == 2 || $this->_role ==3){
			$this->data['title'] ='Giao dịch đổi thẻ';
			$this->data['title_main'] = 'Giao dịch đổi thẻ';
			$this->data['side_bar'] = 5;
			...
			$this->parser->parse('reseller/layout/apps/card_cms',$this->data);
