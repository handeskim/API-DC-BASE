...
class Transfer extends MY_Controller{
	...
	public function info($id){
		if($this->_role == 1 || $this->_role == 2 || $this->_role ==3){
			if(isset($id)){
				...
				$this->obj['keys'] = $id;
				$this->result = $this->GlobalMD->query_result('api/transfer_cms/info',$this->obj);
			...
			$this->parser->parse('reseller/layout/apps/transfer/transfer_info',$this->data);
		}else{
			$this->is_profile();
		}
	}
	public function index(){
		if($this->_role == 1 || $this->_role == 2 || $this->_role ==3){
			$this->data['title'] = lang('transfer_log');
			$this->data['title_main'] = lang('transfer_log');
			$this->data['side_bar'] = 4;
			...
			$this->parser->parse('reseller/layout/apps/transfer_cms',$this->data);
