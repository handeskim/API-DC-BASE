<?php
class Card_chage_config extends MY_Controller{
	...
	public function add(){
		if(!empty($_POST)){
			...
			$add = $this->GlobalMD->query_result('apps/site/card_change_add',$this->obj);
			redirect(base_url('reseller/card_chage_config.html'));
		}else{
			redirect(base_url('reseller/card_chage_config.html'));
		}
	}
	...
	public function index(){
		if($this->_role == 1 || $this->_role == 2 ){
			$this->obj['token'] = $this->_token ;
			$this->obj['client_id'] = $this->client_id ;
			$this->data['title'] = "cấu hình thẻ đổi";
			$this->data['title_main'] = "cấu hình thẻ đổi";
			...
			$this->parser->parse('reseller/layout/apps/card_chage_config',$this->data);
