...
class Cart extends MY_Controller{
	...
	public function info($id){
		if($this->_role == 1 || $this->_role == 2 || $this->_role ==3){
			if(isset($id)){
				...
				$this->obj['keys'] = $id;
				$this->result = $this->GlobalMD->query_result('api/cart_cms/info',$this->obj);
			...
			$this->data['title'] = lang('cart');
			$this->data['title_main'] = 'Detail Transaction Cart ID#'.$id;
			$this->data['side_bar'] = 4;
			$this->parser->parse('reseller/layout/apps/cart/cart_info',$this->data);
		}else{
			$this->is_profile();
		}
	}
	public function index(){
		if($this->_role == 1 || $this->_role == 2 || $this->_role ==3){
			$this->data['title'] ='Giao dịch mua thẻ';
			$this->data['title_main'] = 'Giao dịch mua thẻ';
			...
			$this->parser->parse('reseller/layout/apps/cart_cms',$this->data);
