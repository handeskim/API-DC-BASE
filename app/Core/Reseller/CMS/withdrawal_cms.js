...
function Resolver(e){$("#LoadAjax").css("display","block"),$.ajax({url:BASE_URL+"reseller/service/withdrawal_cms",method:"GET",dataType:"json",async:!0,data:e,success:function(e){$("#LoadAjax").css("display","none"),1==e.status&&TempResolver(e.data)}})}
function Delete_transaction(e){
	var f = "reseller/service/withdrawal_cms_del";
...
function Agreed_transactions(e){
	var f = "reseller/service/withdrawal_cms_agree";
	var p = {csrf_hk_token:token_csrf,e:e};
...
function Cancel_transaction(e){
	var f = "reseller/service/withdrawal_cms_cancel";
	var p = {csrf_hk_token:token_csrf,e:e};
...
function info_ask(e){
	LFactory_load('reseller/withdrawal/info/'+e);
}
function TempResolver(e){
	...
	$.each(e, function(k, v) {
		...
		if(v.transaction==='hold'){
			var action_transfer = '<a title="Xác Thực withdrawal" class="btn btn-small btn-success" onclick="info_ask(\'' + v.id + '\')"><i class="fa fa-exchange"></i></a>';
		}else{
...
function TableResolver(e){
	var table = $('#TableSLData').DataTable({
		"dom": 'Blfrtip',
		 "buttons": [ 
			{ extend: 'copy', 'footer': false, exportOptions: { columns: [ 1,2,3,4,5,7,8,11,12,13,14 ] } },
			{ extend: 'csv', 'footer': false, exportOptions: { columns: [1,2,3,4,5,7,8,11,12,13,14 ] } },
			{ extend: 'excel', 'footer': false, exportOptions: { columns: [ 1,2,3,4,5,7,8,11,12,13,14 ] } },
			{ extend: 'pdf', 'footer': false, exportOptions: { columns: [ 1,2,3,4,5,7,8,11,12,13,14 ] } }
			],
		"destroy": true,
		"async": true,
		"order": [[ 1, "desc" ]],
